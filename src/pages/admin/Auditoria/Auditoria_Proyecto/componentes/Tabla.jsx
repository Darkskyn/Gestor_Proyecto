...
const Tabla = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch('http://localhost:1337/api/auditoria-proyectos', {
        headers: {
  ...
  const filteredData = auditoriaData.filter(item => {
    const nombreProyecto = item.attributes.Nombre_Proyecto || '';
    const usuario = item.attributes.Usuario || '';
    const accion = item.attributes.Accion || '';
    const lowerCaseSearchTerm = searchTerm.toLowerCase();
    
    return (
      nombreProyecto.toLowerCase().includes(lowerCaseSearchTerm) ||
      ...
      <div className="w-full pl-[800px]">
        <div className="relative mt-16 w-full">
          <input
            ...
            placeholder="Buscar por usuario, acción o nombre de proyecto..."
            value={searchTerm}
              ...
              <table className="w-full table-auto border-collapse border border-gray-800">
                <thead className="bg-blue-700 text-white text-base font-semibold uppercase">
                  <tr>
                    ...
                    <th className="p-4 text-center">Nombre Proyecto</th>
                    ...
                    currentItems.map(item => {
                      ...
                      return (
                        <tr key={item.id}>
                          ...
                          <td className="p-4 text-center">{item.attributes.Nombre_Proyecto || 'N/A'}</td>
